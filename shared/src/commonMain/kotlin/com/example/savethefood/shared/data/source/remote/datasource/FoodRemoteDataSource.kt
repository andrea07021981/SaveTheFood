...
class FoodRemoteDataSource(
    ...
    override suspend fun getFoodByQuery(barcode: String): ActionResult<FoodSearchDomain>?  = coroutineScope{
        try {
            ...
            //Log.d("JSON RESULT", foodData.id.toString())
            return@coroutineScope ActionResult.Success(foodData.asDomainModel())
        } catch (e: Exception) {
            return@coroutineScope ActionResult.ExError(e)
        }
    }

    @Deprecated("No network now")
    override suspend fun getFoodById(id: Int): ActionResult<FoodDomain> = coroutineScope{
        try {
            val foodData = client.getFoodById(id)
            return@coroutineScope ActionResult.Success(foodData.asDomainModel())
        } catch (e: Exception) {
            return@coroutineScope ActionResult.Error(e.toString())
        }
    }
